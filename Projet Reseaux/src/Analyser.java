...
public class Analyser extends Application
{
    ...
    // TCP
        for (i = 34 + header_ipv4 - 20; i < 54; i++)	//un header TCP a 20 bytes
            aux.add(frame.get(i));
        ...
        if (header_length * 4 > 20) {
            for (i = 54; i < header_length * 4 + 34; i++) {
                aux.add(frame.get(i));
            }
            GUI.println(myTCP.decode_options_TCP(aux));
        }
        aux.clear();

    // HTTP
        for (i = header_length * 4 + 34; i < frame.size(); i++) {
            aux.add(frame.get(i));
        }
    ...
    public static void readFrame(String fileName) throws Exception
    {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (line.isEmpty())
                ...
                words = line.split("\\s+");    //split the line in bytes
            }
            if (!isHex(words[0]) || !isHex(words[1])) {
                ...
                if (words[0].equals("0000")) {	//new frame
                    ...
                    for (int i = 1; i < words.length; i++)
                        ...
                        if (offset != current_offset && ok == 1) {
                            GUI.println("Error, offset not correct at line: " + current_line + ". The frame will be ignored.\n___________________________________________________________________\n");
                            Alert alert = new Alert(AlertType.ERROR);
                            alert.setTitle("Error dialog");
                            alert.setHeaderText("Corrupt trace");
                            alert.setContentText("Ooops, the offset on line " + current_line + " is not correct. The file might be corrupted. The frame will be ignored");
                            alert.show();
